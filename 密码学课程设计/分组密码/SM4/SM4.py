#ECB模式的SM4加解密

import re
import base64 

SboxTable = [
            ,
        ]

Roundk=[]
Ck=[]

def xor(str1, str2):  # 异或运算
    r = ''
    for i in range(len(str1)):
        t = int(str1[i]) ^ int(str2[i])
        if (int(t) == 1):
            r += '1'
        else:
            r += '0'
    return r

def strTobits(str): #ASCII转2进制
    r = ''
    for i in str:
        t = bin(ord(i))[2:]
        for j in range(0, 8 - len(t)):
            t = '0' + t  # 把输出的b给去掉
        r += t
    return (r)

def binTostr(bin_str):
    r = ""
    tmp = re.findall(r'.{8}', bin_str) #8个字符为一组
    for i in tmp:
        r+= chr(int(i, 2))
    return r


def intTobit(x,n): #十进制转为n位2进制
    r=bin(x)[2:]
    while(len(r)<n):
        r='0'+r
    return r

def move(k, n):  # 循环移位
    return k[n:]+k[:n]      #移位数在长度之内，未做溢出判断

def T(x):  #x:32bits,
    B=''                        #B:32bits
    for i in range(0,8,2):    #分为四组
        B+=intTobit(SboxTable[int(x[i*4:(i+1)*4],2)*16+int(x[(i+1)*4:(i+2)*4],2)],8) #高四位为排，低四位为列。
    C=xor(B,xor(move(B,2),xor(move(B,10),xor(move(B,18),move(B,24)))))
    return C

def TT(x):
    B = ''  # B:32bits
    for i in range(0, 8, 2):  # 分为四组
        B+=intTobit(SboxTable[int(x[i*4:(i+1)*4],2)*16+int(x[(i+1)*4:(i+2)*4],2)],8)  # 高四位为排，低四位为列。
    C = xor(B,xor(move(B,13),move(B,23)))
    return C

def f(x):  #   Rk:32bits
    t = ''
    x=strTobits(x)       #字符串转为比特
    #x=intTobit(int('0x0123456789abcdeffedcba9876543210',16),128)
    for j in range(32):
        t = xor(x[j*32:(j+1) * 32], T(xor(x[(j+1) * 32:(j+2) * 32], xor(x[(j+2) * 32:(j+3) * 32], xor(Roundk[j], x[(j+3) * 32:(j+4) * 32])))))  #x128bits，分为四组进行异或操作。
        x+=t
    return  x[35*32:36*32]+ x[34* 32:35* 32] +x[33* 32:34* 32]+x[32*32:33 * 32]       #倒序输出


def ff(x):
    t = ''
    for j in range(32):
        t = xor(x[j*32:(j+1) * 32], T(xor(x[(j+1) * 32:(j+2) * 32], xor(x[(j+2) * 32:(j+3) * 32], xor(Roundk[j], x[(j+3) * 32:(j+4) * 32])))))  ##x128bits，分为四组进行异或操作。
        x+=t
    return  x[35*32:36*32]+ x[34* 32:35* 32] +x[33* 32:34* 32]+x[32*32:33 * 32]      # 倒序输出

def set_Ck():   #Ck[i]:32bits
    for i in range(32):
        t=''
        for j in range(4):
            t+=intTobit(7*(4*i+j)%256,8)
        Ck.append(t)

def RoundKey(key):  #key:128bits
    key=strTobits(key)
    #key=intTobit(int('0x0123456789abcdeffedcba9876543210',16),128)
    K=[]
    FK1, FK2, FK3, FK4 = 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
    K.append(xor(intTobit(FK1,32),key[0:32]))    #k0
    K.append(xor(intTobit(FK2,32),key[32:64]))   #k1
    K.append(xor(intTobit(FK3,32),key[64:96]))   #k2
    K.append(xor(intTobit(FK4,32),key[96:128]))  #k3
    for i in range(32):
        t=xor(K[i],TT(xor(K[i+1],xor(K[i+2],xor(K[i+3],Ck[i])))))
        K.append(t) #K[4]-K[35]
        Roundk.append(t)    #加入轮密钥

def StrToBase64(str1):
    b = base64.b64encode(str1.encode('latin')).decode('latin')
    return b

def Base64ToStr(b):
    c = base64.b64decode(b.encode('latin')).decode("latin")
    return c

def encrypt(message,key):
    r=''
    set_Ck()
    RoundKey(key)
    m=message
    if m == '0x0123456789abcdeffedcba9876543210':
            r+=f(intTobit(int(m,16),128))
            print(hex(int(r,2)))
    else:
        while(len(m)%16!=0):
            m+='0'
        for i in range(0,len(m),16):
            r+=f(m[i:i+16])                         #分组加密|m为字符串
        return r

def decrypt(message,key):
    r=''
    set_Ck()
    RoundKey(key)
    Roundk.reverse()        #解密密钥是加密密钥的逆序
    m = message
    for i in range(0,len(m),128):
        r+=ff(m[i:i+128])
    r=binTostr(r)

    return r


def main():
    while True:
        mode=input("加密输入e，解密输入d,输入n退出\n")
        if mode=='e':
            message=input('请输入明文：')
            key=input('请输入16字节密钥(例如abcdefghijklmnop)：')
            if len(key)!=16:
                print('密钥长度错误：当前输入密钥长度为'+str(len(key)))
                continue
            result=encrypt(message,key)
            print('密文ASCII形式为：'+binTostr(result))
            print('密文2进制形式为：'+result)
            print('密文base64形式：'+StrToBase64(binTostr(result)))
        elif mode=='d':
            message=input('请输入密文2进制形式:')
            key=input('请输入16字节密钥：')
            message=message
            result=decrypt(message,key)
            print('明文为：'+result)
        elif mode=='n':
            break


if __name__ == '__main__':
    main()
