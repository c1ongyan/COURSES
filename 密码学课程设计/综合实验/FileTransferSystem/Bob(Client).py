...
import AES_ECB_CBC as AES
import rsa as RSA
##########################################################################################
CLIENT_WORKDIR = "D:\\Course\\密码学课程设计\\综合实验\\FileTransferSystem\\ClientData\\"
CLIENT_PUB_PATH = "D:\\Course\\密码学课程设计\\综合实验\\FileTransferSystem\\ClientKeys\\client.pub"
CLIENT_KEY_PATH = "D:\\Course\\密码学课程设计\\综合实验\\FileTransferSystem\\ClientKeys\\client.key"
SERVER_PUB_PATH = "D:\\Course\\密码学课程设计\\综合实验\\FileTransferSystem\\ClientKeys\\server.pub"
MAXBUFFSIZE = 51200
BUFFSIZE = 40960
BUFFSIZE_SENT = 20480
...
def file_encrypt(filepath):
    """
    对文件进行AES-CBC加密,并用RSA加密key和iv
    :param filepath: 原文件路径
    :return: 加密文件路径, 原文件md5值, 加密后的key, 加密后的iv
    """
    # 随机生成key和iv
    key, iv = get_aes_param()
    # 打开原文件
    ...
    encrypted_file.write(AES.CBCEncrypt(origin_filedata, key, iv).encode('latin'))
    # 关闭文件
    origin_file.close()
...
def file_decrypt(encrypted_filepath, key_encrypted, iv_encrypted):
    ...
    key = RSA.decrypt(key_encrypted, client_d, client_n)
    iv = RSA.decrypt(iv_encrypted, client_d, client_n)
    # 打开并读取加密文件
    ...
    origin_file.write(AES.CBCDecrypt(encrypted_filedata, libnum.n2s(key), libnum.n2s(iv)).encode('latin'))
    # 关闭文件指针
    encrypted_file.close()
    origin_file.close()
    # 计算解密后文件的md5消息摘要
...
def transfer_encrypt(message):
    """
    对分组传输的每一个分组进行aes_cbc加密,且每次使用不同的key和iv
    :param message: 被加密的分组
    :return: key,iv,cipher
    """
    key, iv = get_aes_param()
    cipher = AES.CBCEncrypt(message, key, iv)
...
def transfer_decrypt(cipehr, key_encrypted, iv_encrypted):
    ...
    iv = RSA.decrypt(iv_encrypted, client_d, client_n)
    # 用key和iv对收到的密文分组解密
    message = AES.CBCDecrypt(cipehr, libnum.n2s(key), libnum.n2s(iv))
    return message
